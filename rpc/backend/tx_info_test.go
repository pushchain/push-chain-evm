...
func TestReceiptsFromCometBlock(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			receipts, err := backend.ReceiptsFromCometBlock(rpctypes.NewContextWithHeight(1), resBlock, blockRes, msgs, nil)
			require.NoError(t, err)
...
func TestEthMsgsFromCometBlockSkipStateDBCommitFailure(t *testing.T) {
	...
	msgs, _ := backend.EthMsgsFromCometBlock(rpctypes.NewContextWithHeight(100), resBlock, blockRes)
