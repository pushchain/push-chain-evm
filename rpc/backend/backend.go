...
type EVMBackend interface {
	...
	// push-chain: additionally returns the per-message derived-transaction
	// fields, parallel to the returned messages.
	EthMsgsFromCometBlock(ctx context.Context, block *tmrpctypes.ResultBlock, blockRes *tmrpctypes.ResultBlockResults) (
		[]*evmtypes.MsgEthereumTx, []*types.TxResultAdditionalFields)
	...
	// push-chain: both additionally return the derived-transaction fields
	// recovered from the tx result, nil for ordinary EVM txs.
	GetTxByEthHash(ctx context.Context, txHash common.Hash) (*servertypes.TxResult, *types.TxResultAdditionalFields, error)
	GetTxByTxIndex(ctx context.Context, height int64, txIndex uint) (*servertypes.TxResult, *types.TxResultAdditionalFields, error)
...
func NewBackend(
	...
	b := &Backend{
		...
		// push-chain: source the EVM chain ID from the active EVM chain config
		// rather than app.toml, so it cannot drift from the chain's real ID.
