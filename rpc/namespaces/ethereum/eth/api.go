...
func (e *PublicAPI) GetPendingTransactions() (_ []*rpctypes.RPCTransaction, err error) {
	ctx, span := tracer.Start(context.Background(), "eth_getPendingTransactions")
	defer func() { evmtrace.EndSpanErr(span, err) }()
	e.logger.Debug("eth_getPendingTransactions")

	txs, err := e.backend.PendingTransactions(ctx)
	if err != nil {
		return nil, err
	}
