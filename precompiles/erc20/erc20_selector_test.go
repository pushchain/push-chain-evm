...
// runPrecompile drives erc20.Precompile.Run with a nil StateDB and returns the
...
func runPrecompile(t *testing.T, input []byte, value *uint256.Int) string {
	...
	bz, err := erc20.NewPrecompile(erc20types.TokenPair{}, nil, nil, nil).Run(evm, contract, false)
...
func TestRunResolvesSelectorBeforeNativeAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:               erc20.ABI.Methods[erc20.NameMethod].ID,
