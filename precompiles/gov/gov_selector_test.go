...
// runPrecompile drives gov.Precompile.Run with a nil StateDB and returns the
...
func runPrecompile(t *testing.T, input []byte, value *uint256.Int) string {
	...
	bz, err := gov.NewPrecompile(nil, nil, nil, nil, nil).Run(evm, contract, false)
...
func TestRunResolvesSelectorBeforeNativeAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:               gov.ABI.Methods[gov.GetProposalMethod].ID,
