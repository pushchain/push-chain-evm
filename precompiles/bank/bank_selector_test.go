...
// runPrecompile drives bank.Precompile.Run with a nil StateDB and returns the
...
func runPrecompile(t *testing.T, input []byte, value *uint256.Int) string {
	...
	bz, err := bank.NewPrecompile(nil, nil).Run(evm, contract, false)
...
func TestRunResolvesSelectorBeforeNativeAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:               bank.ABI.Methods[bank.BalancesMethod].ID,
