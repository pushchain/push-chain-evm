...
// runPrecompile drives distribution.Precompile.Run with a nil StateDB and returns the
...
func runPrecompile(t *testing.T, input []byte, value *uint256.Int) string {
	...
	bz, err := distribution.NewPrecompile(nil, nil, nil, nil, nil, nil).Run(evm, contract, false)
...
func TestRunResolvesSelectorBeforeNativeAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:               distribution.ABI.Methods[distribution.ValidatorCommissionMethod].ID,
