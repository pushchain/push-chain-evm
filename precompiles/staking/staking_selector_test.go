...
// runPrecompile drives staking.Precompile.Run with a nil StateDB and returns the
...
func runPrecompile(t *testing.T, input []byte, value *uint256.Int) string {
	...
	bz, err := staking.NewPrecompile(nil, nil, nil, nil, nil).Run(evm, contract, false)
...
func TestRunResolvesSelectorBeforeNativeAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:               staking.ABI.Methods[staking.DelegationMethod].ID,
