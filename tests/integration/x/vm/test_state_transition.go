...
func (s *KeeperTestSuite) TestRefundGas() {
	...
	testCases := []struct {
		...
		{
			// A negative gas price must make RefundGas error. RefundGas computes
			// remaining = leftoverGas * gasPrice, so leftoverGas must be > 0 for the
			// negative price to surface (with leftoverGas 0 the product is 0 and no
			// error is raised). leftoverGas == TxGas == transactionGas, so gasUsed is
			// 0 and the expected gas refund is 0.
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			baseFee := unitNetwork.App.GetEVMKeeper().GetBaseFee(ctx)
...
func (s *KeeperTestSuite) TestApplyTransactionWithTxPostProcessing() {
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			// Move the fee collector's real coins into its virtual balance.
			//
			// push-chain: upstream only needs this in the tests where leftover gas is
			// actually refunded. Here leftoverGas is 0, so upstream's refund is a
			// no-op — but push-chain's baseFeeBurn also refunds baseFee * gasUsed
			// before burning it, so this test draws on the fee collector too.
			feeCollectorAddr := authtypes.NewModuleAddress(authtypes.FeeCollectorName)
			err = s.Network.App.GetBankKeeper().SendCoinsFromAccountToModuleVirtual(
				ctx, feeCollectorAddr, authtypes.FeeCollectorName, sdk.NewCoins(sdk.NewCoin("aatom", sdkmath.NewInt(3e18))),
			)
