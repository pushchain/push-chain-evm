...
func (s *TestSuite) TestGetTxByEthHash() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			rpcTx, _, err := s.backend.GetTxByEthHash(s.Ctx(), tc.tx.Hash())
...
func (s *TestSuite) TestGetTransactionByTxIndex() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			txResults, _, err := s.backend.GetTxByTxIndex(s.Ctx(), tc.height, tc.index)
...
func (s *TestSuite) TestQueryCometTxIndexer() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			txResults, _, err := s.backend.QueryCometTxIndexer(s.Ctx(), tc.query, tc.txGetter)
